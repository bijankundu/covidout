    ...
    .then(json => {
      ...
      res.render("home", {
        ...
        world_dead_percentage: Math.floor(
          (Number(world_stats.total_deaths.replace(",", "")) /
            Number(world_stats.total_cases.replace(",", ""))) *
            100
        ),
        ...
        india_active_cases_percentage: Math.ceil(
          (Number(india_status.active) / Number(india_status_total_cases)) * 100
        ),
        india_recovered: india_status.recovered,
        india_recovered_percentage: Math.ceil(
          (Number(india_status.recovered) / Number(india_status_total_cases)) *
            100
        ),
        india_critical: india_status.critical,
        india_critical_percentage: Math.ceil(
          (Number(india_status.critical) / Number(india_status_total_cases)) *
            100
        ),
        india_dead: india_status.deaths,
        india_dead_percentage: Math.ceil(
          (Number(india_status.deaths) / Number(india_status_total_cases)) * 100
        )
      });
    });
});
    ...
    .then(json => {
      ...
      res.render("base", {
        ...
        country_recovered_percentage: Math.ceil(
          (Number(country_stats.recovered) / Number(country_total_cases)) * 100
        ),
        country_critical: country_stats.critical
          .toLocaleString()
          .replace(" ", ","),
        country_critical_percentage: Math.ceil(
          (Number(country_stats.critical) / Number(country_total_cases)) * 100
        ),
